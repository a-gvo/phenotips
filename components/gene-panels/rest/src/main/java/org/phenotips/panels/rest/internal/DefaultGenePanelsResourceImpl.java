...
public class DefaultGenePanelsResourceImpl extends XWikiResource implements GenePanelsResource
{
    ...
    public Response getGeneCountsFromPhenotypes()
    {
        ...
        try {
            ...
            this.logger.error("{} and {} parameters should be integers.", START_PAGE_LABEL, NUMBER_RESULTS_LABEL);
            return Response.status(Response.Status.BAD_REQUEST).build();
        } catch (final ExecutionException | IndexOutOfBoundsException e) {
    ...
    private int determineFirstIndex(final int calculated, final int lastItem)
    {
        if (calculated > lastItem) {
            throw new IndexOutOfBoundsException("The page you requested does not exist.");
        }
        return calculated;
    }
